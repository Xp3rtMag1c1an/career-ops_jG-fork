# Josh Groves

**Customer Success · Sales Operations Veteran · AI Builder**

Scottsdale, AZ (Remote, USA) · jcg23@icloud.com · 303.503.4595
linkedin.com/in/josh-groves · writenode.app

---

## Summary

Sales operator turned AI builder. I scaled Avenue Alive Direct to **$300K+ Y1 with the highest install retention in the company** and authored a **national sales playbook adopted by 400 reps across 40 markets.** I then ran a **$63M loan pipeline at 94% CSAT** for four years at Pulte Mortgage. I now build with the AI stack daily as the solo founder of writenode. I bring three things to Close: native fluency in sales operations and CRM workflows, proven retention metrics, and the AI builder's instinct for what customers actually need.

---

## Why Close

- **I'm a sales operator at heart** — Avenue Alive Direct, Pulte Mortgage = a decade in sales-led organizations
- **Sales-led operator** — eight years in high-touch sales roles across Avenue Alive and Pulte
- **AI builder credentials** — writenode is live in production with peer-level technical fluency
- **Remote-first** matches how Close already operates

---

## Experience

### Founder & Developer — writenode · Dec 2024 – Present
- Built production AI Chrome extension solo without engineering background
- **Architecture:** SOURCE NODE agent · KAIROS classifier · MCP integration
- **Stack:** React, TypeScript, Cloudflare Workers, Vectorize, Gemini 2.5 Pro, Deepgram, AssemblyAI
- YC Spring 2026 application · Hindsight/Vectorize partnership · Don Fotsch (former Apple) championing

### Senior Loan Consultant — Pulte Mortgage · Dec 2019 – Nov 2023

**Sales-led role at $63M scale.**

- **$63M annual loan volume · 180-loan concurrent pipeline · 94% CSAT**
- **Top 3 of 150 consultants** for four consecutive years
- Built personal operating systems so new consultants could plug in and perform
- Trained and mentored new consultants
- Fully remote execution with self-designed workstreams

### Owner & Managing Partner — Avenue Alive Direct (Verizon FiOS / DirecTV) · 2014 – 2018

**Sales operations at 40-market scale.**

- Scaled to **$300K+ Year 1 revenue** · trained 25+ reps directly
- **Authored national sales playbook adopted by 400 reps across 40 markets**
- **Highest install retention in the company** — built objection-handling framework adopted org-wide
- #1 national ranking from Day One until promotion to management
- Operated inside the sales/lead-management workflow day-to-day

### AI Strategy Consultant — Harvest Investment Strategies · Sep 2025 – Present
- Built AI brand automation systems with prompt engineering frameworks
- LinkedIn content automation pipeline with funnel integration
- **3 prompts, not 30** — lightweight beats comprehensive

### International Liaison & Curriculum Developer — Bridge2Rwanda · 2010–2011
- Built stakeholder-aligned training framework still in use 15+ years later

---

## Skills

**Sales Operations:** CRM workflows · Pipeline management · Lead qualification · Sales playbook design
**Customer Success:** Onboarding · Retention · Adoption · Training & enablement
**AI/Automation:** Cloudflare Workers · Vectorize · Gemini · MCP · Prompt engineering
**Operations:** Pipeline management at scale · Training program design

## Education
**BA, Biblical Studies & Theology** — Ouachita Baptist University · 2005–2010
